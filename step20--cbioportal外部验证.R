...
# 外部验证之生存分析 ---------------------------------------------------------------
rm(list = ls())
cbioportal <- data.table::fread("data/other_dataset/cbioportal-survivaldata.csv")
...
# PFS ---------------------------------------------------------------------
pfs_cbioportal <- data.table::fread("data/other_dataset/Progression_Free_cbioportal.csv")
pfs_cbioportal <- pfs_cbioportal[-c(1:3),] 
colnames(pfs_cbioportal) <- as.character(pfs_cbioportal[1,])
pfs_cbioportal <- pfs_cbioportal[-1,]
pfs_cbioportal |> str()
table(pfs_cbioportal$Status)
pfs_cbioportal <- pfs_cbioportal |> dplyr::mutate(
  group = as.factor(group),
  OS = ifelse(Status == "censored",0,1),
)
colnames(pfs_cbioportal)[6] <- "OS.time"
pfs_cbioportal$OS.time <- as.numeric(pfs_cbioportal$OS.time)
cln_xena1 <- pfs_cbioportal[,c(1,6:8)]
# 以下代码直接运行即可
data1 <- cln_xena1 %>% filter(OS.time<=84)
...
# RFS ---------------------------------------------------------------------
Rfs_cbioportal <- data.table::fread("data/other_dataset/Relapse_Free_cbioportal.csv")
Rfs_cbioportal <- Rfs_cbioportal[-c(1:3),] 
colnames(Rfs_cbioportal) <- as.character(Rfs_cbioportal[1,])
Rfs_cbioportal <- Rfs_cbioportal[-1,]
Rfs_cbioportal |> str()
table(Rfs_cbioportal$Status)
Rfs_cbioportal <- Rfs_cbioportal |> dplyr::mutate(
  group = as.factor(group),
  OS = ifelse(Status == "censored",0,1),
)
colnames(Rfs_cbioportal)[6] <- "OS.time"
Rfs_cbioportal$OS.time <- as.numeric(Rfs_cbioportal$OS.time)
cln_xena1 <- Rfs_cbioportal[,c(1,6:8)]
# 以下代码直接运行即可
data1 <- cln_xena1 %>% filter(OS.time<=84)
...
pp <- ggsurvplot(fit, data=data1, linetype = 1, palette = c("jco"),size=1,
                 ...
                 legend.title = '', legend.labs=c("Mut","Wild"),
                 break.time.by =12,
                 xlim = c(0,84),risk.table = TRUE,
                 risk.table.title = "Patients at risk",
                 ylab = "Relapse Free Survival, %",xlab = "Months",
                 font.x = c(20,"plain","black"),font.y = c(20,"plain","black"),
                 font.tickslab = c(20,"plain","black"),
                 risk.table.fontsize = 6,font.legend =c(20,"plain","black"),
                 font.main = c(20,"plain","black"),pval.size = 8)
